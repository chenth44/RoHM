...
wandb_run = wandb.init(project='gaussian_diffusion_posenet')
...
class GaussianDiffusionPoseNet:
    """
    ...
    def p_sample_with_grad(
        ...
        if grad_type == 'prox':
            ############ calculate cond grad 2d projection loss
            grad_weight_2d_projection = 3e5  # 3e5  2e6 for 100
            if t[0] <= 100:
                grad_2d_projection = model.guide_2d_projection_with_smpl(batch, out, t, compute_grad='x_0')  # [bs, 263]
                out["mean"] = out["mean"].float() + grad_weight_2d_projection * out['variance'] * grad_2d_projection.float()
            ############ calculate cond grad foot skaing
            grad_weight_skating = 1e5  # todo 5e4 for 50
            if t[0] <= 100:  # todo
            ...
            if t[0] <= 50:  # todo
                ...
                new_collision_loss = model.calculate_collision_loss(out['pred_xstart'], batch['transf_matrix'])
                wandb_run.log({'collision_loss': new_collision_loss})
    ...
    def training_losses(self, model, batch, t, noise=None, smplx_model=None, epoch=0,):
        """
    ...
    def eval_losses(self,
                    ...
                    timestep_respacing='',
                    compute_loss=True,
                    smplx_model=None, epoch=0):
        """
        Input:
            model: the model to evaluate loss on.
            grad_type: 'prox'/'amass', different schedules to apply gradient guidance during sampling
                        details hardcoded for different datasets
            early_stop: if True, only sample for 980 steps, skip last 20 steps, hardcoded
        """
        if timestep_respacing == '':
            model_output = self.p_sample_loop(model=model.model, batch=batch, shape=shape, progress=progress,
                                              clip_denoised=clip_denoised,
                                              cond_fn_with_grad=cond_fn_with_grad,
                                              grad_type=grad_type, early_stop=early_stop,
                                              save_intermediate_result=False)
