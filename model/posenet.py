...
class PoseNet(nn.Module):
    ...
    def guide_collision_with_smpl(self, batch, out, denoise_t, compute_grad='x_t'):
        ...
        with torch.enable_grad():
            ...
            collision_loss = self.calculate_collision_loss(x_t, batch['transf_matrix'])
            if collision_loss > 0:
                grad_collision = torch.autograd.grad([-collision_loss], [x_t])[0]
    ...
    def calculate_collision_loss(self, x_t, transf_matrix):
        full_repr_rec = x_t[:, :, 0].permute(0, 2, 1)  # [bs, T, body_feat_dim]
        ...
        for i in range(clip_len):
            ...
            for j in range(bs):
                ...
                bb_min = cur_smpl_output.vertices.min(dim=1)[0].reshape(1, 3).detach() @ transf_mat_inv[:3,
                                                                                         :3].T + transf_mat_inv[:3, 3]
                bb_max = cur_smpl_output.vertices.max(dim=1)[0].reshape(1, 3).detach() @ transf_mat_inv[:3,
                                                                                         :3].T + transf_mat_inv[:3, 3]
                ...
                if inds.any():
                    if inds.sum() <= 500:
                        ...
                        for k in range(inds.sum() // 500 + 1):
                            ...
                            if cur_inds.any():
                                collision_loss_per_frame += self.smplx_model.coap.collision_loss(
                                    self.dataset.scene_vertices[cur_inds].unsqueeze(0) @ transf_mat[:3,
                                                                                         :3].T + transf_mat[:3, 3],
                                    cur_smpl_output, ret_collision_mask=None).sum()
